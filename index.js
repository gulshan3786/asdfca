...
app.post("/login",async(req,res)=>{
  ...
  //   }
  // })
  if(a!="" && a.password!=""){  let z=(a[0]['salt'])
    console.log(z)
    ...
    console.log(y);
    console.log("us",a[0].password);
    if(a==""){
      flag=false;
      // res.json(flag);
    }else if(a[0].password!=y){
      flag=false;
  ...
  // var authquery=` select * from user_registration where email='${email}'`;
  //  let y=connection.query(authquery,(err,result)=>{
  //   if(err)throw err;
  //   const res1=result[0];
  //   console.log(res1);
  // });
